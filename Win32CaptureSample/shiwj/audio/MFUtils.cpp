...
//void MFUtils::SafeReleaseSample(IMFSample* sample)
//{
//	if (sample)
//	{
//		IMFMediaBuffer* buf;
...
bool MFUtils::Startup()
{
	...
	{
		//XTrace(LOG_ERROR, L"Failed to initialize COM.\n");
		//return false;
	}
	...
	if (FAILED(hr))
	{
		XTraceE(L"Failed to initialize Media Foundation");
		return false;
	}
...
IMFSample* MFUtils::CreateVideoSampleFromNV12(RAWVideoFrame* frame)
{
	if (frame == nullptr)
		return nullptr;
	...
	const DWORD cbBuffer = frame->Info.Stride * frame->Info.Height;
	...
	if (SUCCEEDED(hr))
	{
		hr = MFCopyImage(
			...
			frame->Info.Stride,                    // Destination stride.
			(BYTE*)frame->Buffer,    // First row in source image.
			frame->Info.Stride,                    // Source stride.
			frame->Info.Stride,                    // Image width in bytes.
			frame->Info.Height                // Image height in pixels.
		);
	}
...
IMFSample* MFUtils::CreateAudioSampleFromPCM(RAWAudioFrame* frame)
{
	if (frame == nullptr)
		return nullptr;
	...
	if (SUCCEEDED(hr))
	{
		...
		//hr = MFCopyImage(
		//	pData,                      // Destination buffer.
		//	frame->Stride,                    // Destination stride.
		//	(BYTE*)frame->Buffer,    // First row in source image.
		//	frame->Stride,                    // Source stride.
		//	frame->Stride,                    // Image width in bytes.
...
IMFSample* MFUtils::CopyVideoSample(IMFSample* srcSample, int width, int height, int stride)
{
	...
	const DWORD cbBuffer = stride * height;
	...
	if (SUCCEEDED(hr))
	{
		hr = MFCopyImage(
			...
			(BYTE*)srcBufferData,    // First row in source image.
...
IMFSample* MFUtils::CopyAudioSample(IMFSample* srcSample)
{
	if (srcSample == nullptr)
		return nullptr;
	...
	// Create a new memory buffer.
	hr = MFCreateMemoryBuffer(cbCurrentLength, &newBuffer);
	...
	if (SUCCEEDED(hr))
	{
		memcpy(pData, (BYTE*)srcBufferData, cbCurrentLength);		
	}
	...
	if (SUCCEEDED(hr))
	{
		hr = newBuffer->SetCurrentLength(cbCurrentLength);
	}
...
void MFUtils::GetVideoMediaType(IMFMediaType* srcMediaType, VFVideoMediaType* outputMediaType)
{
	//Type = mediaType;

	memset(outputMediaType, 0, sizeof(VFVideoMediaType));

	HRESULT hr = srcMediaType->GetGUID(MF_MT_SUBTYPE, &outputMediaType->SubType);
	hr = MFGetAttributeSize(srcMediaType, MF_MT_FRAME_SIZE, &outputMediaType->Width, &outputMediaType->Height);

	hr = MFGetAttributeRatio(srcMediaType, MF_MT_FRAME_RATE, &outputMediaType->FrameRateNum, &outputMediaType->FrameRateDen);
//	outputMediaType->FrameRate = (double)outputMediaType->FrameRateNum / outputMediaType->FrameRateDen;

	hr = MFGetAttributeRatio(srcMediaType, MF_MT_FRAME_RATE_RANGE_MIN, &outputMediaType->FrameRateMinNum, &outputMediaType->FrameRateMinDen);
//	outputMediaType->FrameRateMin = (double)outputMediaType->FrameRateMinNum / outputMediaType->FrameRateMinDen;

	hr = MFGetAttributeRatio(srcMediaType, MF_MT_FRAME_RATE_RANGE_MAX, &outputMediaType->FrameRateMaxNum, &outputMediaType->FrameRateMaxDen);
//	outputMediaType->FrameRateMax = (double)outputMediaType->FrameRateMaxNum / outputMediaType->FrameRateMaxDen;

	if (outputMediaType->SubType == MFVideoFormat_YUY2)
	{
		wsprintfW(outputMediaType->SubTypeString, L"YUY2");
	}
	else if (outputMediaType->SubType == MFVideoFormat_RGB24)
	{
		wsprintfW(outputMediaType->SubTypeString , L"RGB24");
	}
	else if (outputMediaType->SubType == MFVideoFormat_RGB32)
	{
		wsprintfW(outputMediaType->SubTypeString , L"RGB32");
	}
	else if (outputMediaType->SubType == MFVideoFormat_NV12)
	{
		wsprintfW(outputMediaType->SubTypeString , L"NV12");
	}
	else if (outputMediaType->SubType == MFVideoFormat_MJPG)
	{
		wsprintfW(outputMediaType->SubTypeString , L"MJPG");
	}
	else if (outputMediaType->SubType == MFVideoFormat_I420)
	{
		wsprintfW(outputMediaType->SubTypeString , L"I420");
	}
	else
	{
		wsprintfW(outputMediaType->SubTypeString , L"UNKNOWN");
	}

	wsprintfW(outputMediaType->Name, L"%dx%d %ls %d/%d fps", outputMediaType->Width, outputMediaType->Height, outputMediaType->SubTypeString, outputMediaType->FrameRateNum, outputMediaType->FrameRateDen);
}

void MFUtils::LogSample(IMFSample* sample)
{
	LONGLONG timestamp = 0, duration  = 0;
	HRESULT hr = sample->GetSampleTime(&timestamp);
	hr = sample->GetSampleDuration(&duration);

	XTraceE(L"Sample %lld ms at %lld ms\n", duration / 10000, timestamp / 10000);
}
...
//};
//static const DWORD PreferredVideoSubTypeGuidPairsCount = sizeof(PreferredVideoSubTypeGuidPairs) / sizeof(PreferredVideoSubTypeGuidPairs[0]);
//
//// Video Processor
...
//	XTraceE("D3D9 supported");
//
//bail:
//	if (!MFUtils::g_bD3D9Supported) {
//		XTraceE("D3D9 not supported");
//	}
//	SafeRelease(&pDevice);
//	SafeRelease(&pD3D);
//	return MFUtils::g_bD3D9Supported;
...
//}
//
HRESULT MFUtils::CreateMediaSample(DWORD cbData, IMFSample **ppSample)
{
	assert(ppSample);
	...
	TESTHR(hr = MFCreateSample(&pSample));
	TESTHR(hr = MFCreateMemoryBuffer(cbData, &pBuffer));
	TESTHR(hr = pSample->AddBuffer(pBuffer));
...
//			{
//				XTraceE("Using Intel Quick Sync encoder :)");
//				return hr;
//			}
//
//			XTraceE("Not using Intel Quick Sync encoder :(");
//		}
//		else
//		{
...
//			if (SUCCEEDED(hr) && *ppMFT)
//			{
//				XTraceE("Using Intel Quick Sync decoder :)");
//				return hr;
//			}
//
//			XTraceE("Not using Intel Quick Sync decoder :(");
//		}
//	}
//
//	UINT32 count = 0;
...
//					XTraceE("MS H.264 encoder is disabled...skipping");
//					continue;
//				}
//
...
//					XTraceE("MS H.264 encoder doesn't support low delay on (%ld, %ld)...skipping", g_dwMajorVersion, g_dwMinorVersion);
//					continue;
//				}
//			}
//		}
...
//					XTraceE("MS H.264 decoder doesn't support low delay on (%ld, %ld)...skipping", g_dwMajorVersion, g_dwMinorVersion);
//					continue;
//				}
//			}
//		}
...
//				if (bAsync)
//				{
//					XTraceE("Skipping async decoder because not supported yet");
...
//		XTraceE("Session error: 0x%x (event id: %d)\n", hrStatus, met);
//		hr = hrStatus;
//		goto bail;
//	}
//
...
//		*pnHeight = 480;
//		*pnFps = 30;
//		XTraceE("Failed to math subtype...using VGA@30fps");
//	}
//

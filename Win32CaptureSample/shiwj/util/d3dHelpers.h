...
namespace util::uwp
{
    ...
    inline std::vector <IDXGIAdapter*> EnumerateAdapters(void)
    {
            ...
            ++i)
        {
            ...
            LOG_INFO(L"DXGI desc:%s", desc.Description);
        }
    ...
    inline auto CreateD3DDevice(D3D_DRIVER_TYPE const type, winrt::com_ptr<ID3D11Device>& device,int index = 0)
    {
        WINRT_ASSERT(!device);
    ...
    inline auto CreateD3DDevice(int index = 0)
    {
        winrt::com_ptr<ID3D11Device> device;
        HRESULT hr = CreateD3DDevice(D3D_DRIVER_TYPE_HARDWARE, device, index);

...
MonitorInfo  MonitorList::GetMonitorByName(std::wstring displayname)
{
    std::vector<MonitorInfo>::iterator iter = m_monitors.begin();
    while (iter!= m_monitors.end())
    {
        if(std::equal(displayname.begin(), displayname.end(),iter->DisplayName.begin()))
            return *iter;
        iter++;
    }
    return nullptr;
...
MonitorInfo* MonitorList::GetDisplayMonitor()
{
    std::vector<MonitorInfo>::iterator iter = m_monitors.begin();
    while (iter != m_monitors.end())
    {
